...
- Added point-group CRUD and its bridge families under `/mgd/gateways/{id_setup}/devices/{child_id_setup}/point-groups` (keyed by `point_group_name`, with relay_control device-config resolution) plus a gateway special-days catalog (`/mgd/gateways/{id_setup}/special-days`). A shared validator enforces one `everyday` and one `special_day` behavior per device config.
