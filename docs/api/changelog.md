...
### history
- Standardized the public temporal filters for collaborator, device, and
  monitor history endpoints on inclusive UNIX-second `from` and `to` parameters.
  Monitor history keeps `from_timestamp` and `to_timestamp` as hidden legacy
  compatibility aliases.

### docs
- Refreshed the history endpoint guides and maintained Postman collections to
  match the production API contract, including defaults and inverted-range
  validation.

## Unreleased (2026-09-07)

### mgd
- Documented that complete-snapshot imports require `json_version >= v4.1` and
  accept only the v4.3 list form for per-device `special_days`.
- Documented the `special_days_invalid_format` warning in non-strict imports,
  the `400` strict response, component-level special-day mappings,
  `channel_configs` component settings, ignored `connection.resource`, and the
  existing `model_point_catalog_missing` behavior.
...
- Replaced stale device-model point examples, added canonical MGD point
  requests to the maintained Postman collections, and synchronized the
  documentation indexes.
- Audited the remaining AP-v4 guides against the active route table, added
  device-setup uninstall and complete EAV endpoint coverage, removed stale
  component-scoped point requests from maintained collections, and marked the
  unregistered time-pattern page as reference-only.
- Corrected EAV and setup-metric authorization notes, completed model-setting
  response documentation, and removed retired MGD device routes and unsupported
  query parameters from all maintained Postman collections.

## Unreleased (2026-08-25)

### mgd
- Documented that operators, uninstall, and `force` imports can cancel
  `pending` changes. The API compensates Clickie under the row lock, and a
  worker transition that arrives after cancellation receives `409`.
- Documented optional `scheduled_at` persistence on individual and bulk status
  PATCH requests; omitted values preserve the existing row and `null` clears it.
- Documented complete-snapshot imports: present MGD rows are upserted, absent
  managed rows are deleted in the gateway, and effective creates, updates, and
  deletes are recorded as `applied` without changing history or global catalogs.

### docs
- Synchronized the MGD state, endpoint, JSON projection, and persistence guides
  with the API publication `deploy/api/20260825T150811Z` (`ap-v4-mgd`).
- Refreshed both published Postman collections and the roadmap coverage for the
  updated cancellation, status timestamp, and importer behavior.

## Unreleased (2026-08-21)

### mgd
- Replaced proposal-only persistence with eager write: operational mutations
  persist the canonical row and return `202` with a real `id_resource`;
  `applied` confirms the live row; `cancelled` and `failed` compensate
  Clickie.
- Cancellable states are now `on_hold` and `retry` only. `pending` is not
  cancellable because SQS may already have been delivered.
- Documented option-B use-edge promotion: writing `group → schedule` also
  promotes extensions already bound to that schedule; attaching an existing
  extension promotes that extension, not the schedule.
- Documented graph 409s (`resource_has_active_change`,
  `related_resource_has_active_change`), catalog `live_dependency`, per-row
  `config_sync`, gateway snapshot apply, and `include_children` honored only
  on the gateway reconcile path.
- `failed` is terminal compensation after the worker asserts the device does
  not have the new value; `failed_change_count` is telemetry.

### docs
- Rewrote the MGD guides for eager persist, added the device-config identity
  page, and enabled Mermaid rendering on GitHub Pages through Primer's
  `head-custom.html` hook.

## Unreleased (2026-08-13)

### mgd
- Canonical child setup responses now report `has_device_config` and
  `id_setup_gateway_device_config_ids` without embedding device-config fields;
  the nested `/devices/{child_id_setup}/configs/{config_id}` routes remain the
  singular config resolvers.
- Added the canonical MGD route/identity matrix, assembled JSON projection,
  proposal-storage design, cancellation/retention rules, direct SQS replay
  contract, and dump-based endpoint sweep evidence.
- Documented strict proposal-only CRUD with nullable `id_resource`, logical
...
- The former component-scoped `/mgd/gateways/{id_setup}/devices/{child_id_setup}/points/{name}` route is superseded by the canonical `/configs/{config_id}/points/{id_device_model_point}` route; maintained collections now expose only the canonical path.
