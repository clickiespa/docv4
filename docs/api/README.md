...
### `mgd`
- [MGD gateway configuration endpoints](gateways/mgd_gateways.md) — `/mgd/gateways`: template CRUD and apply, component/protocol/schedule-type catalogs, gateway configuration, components, child-device bindings, model points keyed by canonical IDs, point groups and their point/special-day bridges, special days, schedules using numeric scope IDs, extensions, config-change list/status endpoints, reconciliation, and canonical JSON imports.
