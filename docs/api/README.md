...
### `mgd`
- [MGD gateway configuration endpoints](gateways/mgd_gateways.md) — `/mgd/gateways`: templates (with `apply`), component/protocol catalogs, gateway configuration, components, component-scoped devices and points, point groups and their point/special-day bridges, component special-day catalog, schedules, schedule bindings, schedule extensions, config-change list/status endpoints, and canonical JSON imports used by the platform configuration UI.
- [MGD gateway smoke tests](gateways/mgd_gateway_smoke_tests.md) — route-level smoke coverage for the MGD API, including execution instructions, request examples, validation cases, and expected response envelopes.
