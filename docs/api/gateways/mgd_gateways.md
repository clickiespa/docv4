...
Point writes evaluate the active-point rule for their component. When a point
mutation enters `pending`, related `on_hold` rows are promoted within the FK
branch and one SQS notification is sent after commit. Unrelated branches,
idempotent writes and `on_hold` point writes do not notify.
...
| Field | Required | Type | Default | Description |
| --- | --- | --- | --- | --- |
| `change_status` | Yes | string | No | Target status (`on_hold`, `pending`, `in_progress`, `retry`, `cancelled`, `applied`, `failed`) for the selected config change |

Bulk update:

| Field | Required | Type | Default | Description |
| --- | --- | --- | --- | --- |
| `change_status` | Yes | string | No | Target status (`on_hold`, `pending`, `in_progress`, `retry`, `cancelled`, `applied`, `failed`) for every selected row |
