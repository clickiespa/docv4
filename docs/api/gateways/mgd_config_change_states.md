...
The API contract covered here is the ClickieV4 `4.2.1` contract. Database
definition changes are owned by the canonical `clickie-platform` database
branch; the SQL files under this legacy repository's `assets/mgd/Archive/`
folder are historical references and are not deployment instructions.
...
by the API and reconciliation flow. In particular, `pending -> on_hold` is
only performed by reconciliation after a target loses its qualifying main
installation; it is not available to an operator or the external worker.
...
If a previously `pending` target is no longer installed, reconciliation
demotes it to `on_hold` so it cannot be dispatched against an absent device.
