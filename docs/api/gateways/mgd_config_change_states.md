...
or `applied`, and `cancelled` only from `retry`. AP-v4 owns `on_hold` and
`pending`. Repeating the current status is idempotent.
...
Cancellable states are `{on_hold, retry}` only. `pending` is not cancellable:
SQS may already have been delivered and the worker may already have started on
the device. Compensating Clickie in that window would create drift.
`in_progress` is not cancellable either.
...
`cancelled` means the user gave up. Demotions `pending → on_hold` do not
compensate: Clickie keeps the eager value.
...
Only `on_hold` and `retry` can become `cancelled`. To abandon a `pending`
change, wait for the worker (`applied`, `failed`, or `retry`). On `retry` the
worker has said it will try again and has not yet returned to `in_progress`,
so cancel is allowed. Uninstall and import with `force=true` return `409` if
any row is `pending` or `in_progress`; they cancel only `on_hold` and
`retry`.
...
Uninstall, import `force`, and reconcile still operate at **setup** scope:
`409` if that target has `pending`/`in_progress`. That is device lifecycle,
not the point-edit mutex.
...
| Setup uninstall | not that PATCH; cancels cancellable rows | yes, `on_hold`/`retry` → `cancelled` | `cancelled` |
| Import `force=true` | `POST .../imports` | yes, only `on_hold`/`retry` → `cancelled` | `cancelled` |
...
### Abandoning `pending` (SQS may already have left)

There is no `cancelled` PATCH. Wait for the worker.

```mermaid
flowchart TD
    pending["pending not cancellable"]
    pending --> applied["applied: sync on new value"]
    pending --> failed["failed: API compensates; sync on previous value"]
    pending --> retry["retry: cancel is allowed"]
    pending --> inprog["in_progress: wait"]
```
...
flowchart TD
    u[Uninstall API]
    u --> p{"pending or in_progress present?"}
    p -->|yes| w409["409 wait_for_worker"]
    w409 --> inst{"target still installed?"}
    inst -->|yes| wait[wait for worker]
    inst -->|no| rec[POST reconcile]
    rec --> demote["pending to on_hold without compensate"]
    demote --> u2[retry uninstall]
    u2 --> cancel["cancel on_hold/retry and compensate"]
    p -->|no| cancel
```
...
flowchart TD
    ...
    same -->|on_hold or retry| a409["409 resource_has_active_change cancel_then_retry"]
    same -->|pending or in_progress| w409["409 resource_has_active_change wait_for_worker"]
...
`force=false` with only `on_hold`/`retry`: `409 cancel_then_retry` (retry with
`force=true`). If any row is `pending` or `in_progress`: `409 wait_for_worker`
...
effective JSON document, upserts/overwrites MGD tables, and records only
effective writes as `applied`. `dry_run` rolls everything back, `strict`
turns flags into errors, and `force` cancels only `on_hold` and `retry`. If
any row is `pending` or `in_progress`, the API returns `409` even with
`force`. The importer is not proposal-only: it writes entities and records
`applied`.
