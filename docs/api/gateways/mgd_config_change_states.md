...
The importer requires `json_version >= v4.1`. Per-device `special_days` must be
the v4.3 list of objects; a legacy dictionary is reported as
`special_days_invalid_format` and skipped in non-strict mode, or returns `400`
with `Per-device special_days must be a list` in strict mode. Component-level
`special_days` remains a mapping for reusable catalog rows. `channel_configs`
lands in component `settings`, while `connection.resource` is ignored as a
legacy field. `model_point_catalog_missing` is still emitted only when a model
point reference lacks enough metadata to register the catalog point.
