...
| Method and path | Meaning |
| --- | --- |
| `GET /mgd/gateways/{id_setup}/config-changes` | Paginated history ordered by `created_at DESC, id DESC`. Filters: inclusive UNIX-second `from`/`to` over `created_at`, `change_status`, `change_group_key`, `operation=C\|U\|D`, `id_setup_target`, `limit`, `offset`, and opt-in `include_config_sync=true`. |
...
The importer validates the complete payload before committing. A malformed
component device container, missing physical point `address`/`gpio`,
non-positive `schedule.every`, or relay point group missing
`schedule_type`, `schedule_number`, `config`, or `registers` returns a
structured `400` naming the invalid JSON path. The snapshot is rolled back;
these client-payload errors are not reported as an unexpected `500`.
