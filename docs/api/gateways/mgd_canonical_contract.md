...
current collection-create body accepts `id_device_model_point_ids`; an
atomic body that also carries `factor_override` and `available_status` per
point is a planned contract extension.
