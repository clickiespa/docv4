...
pending -> on_hold (reconcile only when the main installation is lost)
                                      \-> applied
                                      \-> failed
```
...
effective writes and reconciliation. Reconciliation also distinguishes a
missing or accessory installation from a qualifying main installation before
promoting or demoting a change.
