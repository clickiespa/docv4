...
| `from` | No | integer or null | null | Inclusive UNIX timestamp in seconds; filters `created_at` from this value onward. |
| `to` | No | integer or null | null | Inclusive UNIX timestamp in seconds; filters `created_at` through this value. |
...
```bash
curl -X GET -H "Authorization: <API_KEY>" -H "Account: <ID_ACCOUNT>" \
  "/mgd/gateways/501234/config-changes?from=1788307200&to=1788393600&limit=100&offset=0&include_config_sync=false"
```

The `from` and `to` bounds are inclusive and apply to `created_at`. Either
bound may be omitted; an inverted range returns `400` without exchanging the
values. The filters can be combined with status, group, operation, target,
sync-hint, and offset parameters.
