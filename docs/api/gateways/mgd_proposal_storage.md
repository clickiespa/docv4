...
The SQS race is serialized by the lock. Cancelling `pending` compensates
Clickie in the same transaction; if the worker receives `409` when moving to
`in_progress` or `applied`, it abandons the message and does not retry until
`applied`.
...
`on_hold`, `pending`, and `retry` can become `cancelled`. `in_progress` cannot;
`applied` and `failed` are terminal. Cancel keeps history and compensates the
entity. Cancelling `pending` is safe against the worker: an illegal state
transition returns `409` and the external process terminates. Uninstall and
import with `force` return `409` only if any row is `in_progress`.
...
The importer receives a complete effective snapshot, upserts present MGD
entities and deletes absent entities from the gateway. It records every
effective create, update, or delete as `applied`, preserves history, and does
not modify global catalogs. With `force` it cancels `on_hold`, `pending`, and
`retry`; `in_progress` returns `409`.
